...
namespace Saber.Vendors.Collector.Services
{
    public class CollectorArticles : Service, IVendorService
    {
        ...
        public string Search(int subjectId, int feedId, int domainId, int score, int sort, string search, int start, int length)
        {
            if (!CheckSecurity()) { return AccessDenied(); }
            return Articles.RenderList(subjectId, feedId, domainId, start, length, score, search, Query.Articles.IsActive.Both, false, 0, null, null, (Query.Articles.SortBy)sort);
        }
