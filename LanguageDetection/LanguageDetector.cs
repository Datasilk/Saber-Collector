...
namespace LanguageDetection
{
    public class LanguageDetector
    {
        ...
        {
            var profiles = new DirectoryInfo(Saber.App.MapPath("/Vendors/Collector/LanguageDetection/Profiles"));
            var files = profiles.GetFiles();
            if(files.Length > 0)
            {
                foreach(var file in files)
                {
                    LanguageProfile profile = JsonSerializer.Deserialize<LanguageProfile>(File.ReadAllText(file.FullName));
