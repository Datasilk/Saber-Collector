    ...
    public class DownloadHub : Hub
    {
        ...
        public async Task CheckQueue(string id, int feedId, string domainName, int sort)
        {
            try
            {
                ...
                var queue = Query.Downloads.CheckQueue(feedId, domainName, Domains.DownloadMinimumIntervals, (Query.Downloads.QueueSort)sort); // 60 second delay for each download on a single domain
                if (queue != null)
                {
